...
def simulator_withhomo(turns_list,seed_mut_perlin,generations_list, rec_rate, compare3SEQ,seq_length,temp_file,parallel_prop,seed_gen,seed_gene_homo,num_seeds,len_UAB,mut_rate,sample_rate,qmatrix):
    run_total_number = 0
    result_record = {}
    for genera in generations_list:
        recom_file = temp_file+"output_smp"+str(seed_mut_perlin)+"_sr"+str(sample_rate)+"/"
        ...
        with open(recom_file+"smp"+str(seed_mut_perlin)+"_gener"+str(genera)+"_truns"+str(turns_list)+".csv","a+") as f:
        ...
        for turns in range(1,turns_list_deep+1):
            ...
            if compare3SEQ == True: 
                for s in seq_samples:
                    if "bk" not in s :
                        seq_file0 = recom_file+"seq_method/"
                        creat_dir(seq_file0)
        ...
        df_result_record.to_csv(recom_file+"smp"+str(seed_mut_perlin)+"_"+str(lin_diff_num)+"gener"+str(genera)+"_truns"+str(turns)+"_result"+".csv") 
        run_total_number+=1
...
def main(sysargs=sys.argv[1:]):
    parser = argparse.ArgumentParser(
        ...
        usage='''python Simulator_CovRecombTest.py -smp 6 -sr 1 -f "/home/soniali/Desktop/03_CovRecomb/CovRecomb_Simulation_Test/CovRecomb_compare_3SEQ/"''')
    ...
    tagRun, y_TPR_turns_mean,df_cor_rate_record = simulator_withhomo(turns_list,seed_mut_perlin,\
        generations_list, rec_rate, compare3SEQ,seq_length,temp_file,parallel_prop,seed_gen,\
            seed_gene_homo,num_seeds,len_UAB,mut_rate,sample_rate,qmatrix)
